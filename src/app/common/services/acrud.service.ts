...
import {
  ...
  LikeUser,
  LikeUserDetail,
} from "src/app/common/models/UPost.model";
...
import { Profile, User1 } from "../models/user.model";
...
export class ACrudService {
  ...
  getFeaturedPost() {
    return new Promise((resolve) => {
      this.http
        .get(`https://blogapp-cb4a3-default-rtdb.firebaseio.com/post.json`)
        .subscribe((d) => {
  ...
  getPublicPost(): Observable<UPost[]> {
    return this.http.get<UPost[]>(
      `https://blogapp-cb4a3-default-rtdb.firebaseio.com/post/${this.uid}/public.json`
    );
  }
  ...
  getPrivatePost(): Observable<UPost[]> {
    return this.http.get<UPost[]>(
      `https://blogapp-cb4a3-default-rtdb.firebaseio.com/post/${this.uid}/private.json`
    );
  }

  //fetches all the data from the below realtime database url ;
  ...
  getProfile(): Observable<Profile[]> {
    ...
    if (this.uid) {
      return this.http.get<Profile[]>(
        `https://blogapp-cb4a3-default-rtdb.firebaseio.com/post/${this.uid}/profile.json`
      );
    } else {
  ...
  // Fetching posts from the user uploaded posts
  ...
  getCommentDataFromKey(post_userid, dbkey) {
    return this.http.get(
      `https://blogapp-cb4a3-default-rtdb.firebaseio.com/post/${post_userid}/public/${dbkey}/commentData.json`
    );
  }

  // Fetching comments from different users
  ...
  getPublicProfileKey(value: any, uname: any) {
    return this.http
      .get<Profile[]>(
        `https://blogapp-cb4a3-default-rtdb.firebaseio.com/PublicProfile/${uname}.json`
      )
      .pipe(
        map((responseData) => {
          for (const key in responseData) {
  ...
  // Fetching Profile from user id

  getProfileFromUid(uid): Observable<UPost[]> {
    return this.http.get<UPost[]>(
      `https://blogapp-cb4a3-default-rtdb.firebaseio.com/post/${uid}/profile.json`
    );
  }

  // Fetching Publc Posts from user id
  ...
  // Fetching Private Profile user id
  ...
  //Fetching the like count for the user posts

  getLike(postuserid, title, desc) {
    let dbkey;
    this.http
      .get(
        `https://blogapp-cb4a3-default-rtdb.firebaseio.com/post/${postuserid}/public.json`
      )
      .subscribe((post) => {
        for (const key in post) {
          if (post[key].title == title && post[key].desc == desc) {
        ...
        return this.http.get(
          `https://blogapp-cb4a3-default-rtdb.firebaseio.com/post/${postuserid}/public/${dbkey}/likestatus.json`
        );
      });
  }

  // Fetching posts id for search filter
  ...
  // Fetching post details for like count
  ...
  getPostDetailForLike(postid, title, desc) {
    ...
    x.subscribe((post) => {
      ...
      if (dbkey) {
        this.http
          .get(
            `https://blogapp-cb4a3-default-rtdb.firebaseio.com/post/${postid}/public/${dbkey}.json`
          )
          .subscribe((data: any) => {
  ...
  /* Create methods logically grouped */
  ...
  // Creating new posts by the user
  ...
  // Creatig Private posts by the user
  ...
  // Creating Public posts by the user
  ...
  // Creating Like count for the number of likes by a particular user
  ...
  CreateLikeEntry(likecount, likestatus, postuserid, title, desc) {
    ...
    x.subscribe((post) => {
      ...
      this.http
        .patch(
          `https://blogapp-cb4a3-default-rtdb.firebaseio.com/post/${postuserid}/public/${postid}/likestatus.json`,
          likedata
        )
      ...
      this.PostDataForLikedByUser.subscribe((d) => {
        ...
        if (found) {
          this.http
            .put(
              `https://blogapp-cb4a3-default-rtdb.firebaseio.com/post/${postuserid}/public/${postid}/likestatus/uid/${userlikedetailkey}.json`,
              userdata
            )
            .subscribe((d) => {});
        }
        ...
        if (!found) {
          this.http
            .post(
              `https://blogapp-cb4a3-default-rtdb.firebaseio.com/post/${postuserid}/public/${postid}/likestatus/uid.json`,
              userdata
            )
            .subscribe((d) => {});
        }
  ...
  // Creating comment entry function for the users comment
  ...
  // Functions carried out to implement useful linking
  ...
  UpdateProfile(value, oldvalue, imgdownloadurl) {
    ...
    c.subscribe((d) => {
      ...
      this.http
        .patch(
          `https://blogapp-cb4a3-default-rtdb.firebaseio.com/post/${this.uid}/profile/${this.db_key}.json`,
          this.editedProfileData
        )
        .subscribe((responseData) => {
  ...
  // Deleting the public posts posted by the user

  deletePublicPost(postdata: {}, c: Observable<void>) {
    c.subscribe((x) => {
      this.http
        .delete(
          `https://blogapp-cb4a3-default-rtdb.firebaseio.com/post/${this.uid}/public/${this.db_key}.json`
        )
        .subscribe((d) => {
  ...
  // Deleting the private posts posted by the user

  deletePrivatePost(postdata: {}, c: Observable<void>) {
    c.subscribe((x) => {
      this.http
        .delete(
          `https://blogapp-cb4a3-default-rtdb.firebaseio.com/post/${this.uid}/private/${this.db_key}.json`
        )
        .subscribe((d) => {
          this.router.navigate(["myposts"]);
        });
  ...
  // Deleting further post events carried by the user
